...
A good place to start is parsing types from mscorlib (BCL) with a focus on performance.  It will (eventually) be called at compile time so it needs to be as fast as possible.
